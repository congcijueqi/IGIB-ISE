...
class IndependentCNN_main(nn.Module):
    ...
    def forward(self, x1,x2,x3):
        ...
        channel_std_3 = x2.std(dim=1)
        channel_importance_3, p_3 = self.compress(x2)
        channel_importance_3 = channel_importance_3.unsqueeze(-1)
        
        ib_x2_mean = x2 * channel_importance_3  # 去除 (1 - channel_importance) * channel_means.unsqueeze(1)
        ib_x2_std = (1 - channel_importance_3) * channel_std_3.unsqueeze(1)
        ib_x2 = ib_x2_mean + torch.rand_like(ib_x2_mean) * ib_x2_std
        
         # KL 散度损失计算,先算 ta tm 是ta与他的0 1做kl
        epsilon = 1e-8
        KL_tensor = 0.5 * (
            (ib_x2_std**2) / (channel_std_3.unsqueeze(1) + epsilon)**2 +
            (channel_std_3.unsqueeze(1)**2) / (ib_x2_std + epsilon)**2 - 1
        ) + (ib_x2_mean**2) / (channel_std_3.unsqueeze(1) + epsilon)**2  # 修改了这里，将 (ib_x_mean - 0)**2 替换为 ib_x_mean**2

        KL_Loss = torch.mean(KL_tensor)
        ib_x2 = ib_x2.permute(0, 2, 1)  # 调整维度顺序
        return ib_x2, KL_Loss,p_3
