...
class IndependentCNN_main(nn.Module):
    ...
    def forward(self, x1,x2,x3):
        ...
        channel_std_3 = x1.std(dim=1)
        channel_importance_3, p_3 = self.compress(x1)
        channel_importance_3 = channel_importance_3.unsqueeze(-1)
        
        ib_x1_mean = x1 * channel_importance_3  # 去除 (1 - channel_importance) * channel_means.unsqueeze(1)
        ib_x1_std = (1 - channel_importance_3) * channel_std_3.unsqueeze(1)
        ib_x1 = ib_x1_mean + torch.rand_like(ib_x1_mean) * ib_x1_std
        
         # KL 散度损失计算,先算 ta tm 是ta与他的0 1做kl
        epsilon = 1e-8
        KL_tensor = 0.5 * (
            (ib_x1_std**2) / (channel_std_3.unsqueeze(1) + epsilon)**2 +
            (channel_std_3.unsqueeze(1)**2) / (ib_x1_std + epsilon)**2 - 1
        ) + (ib_x1_mean**2) / (channel_std_3.unsqueeze(1) + epsilon)**2  # 修改了这里，将 (ib_x_mean - 0)**2 替换为 ib_x_mean**2

        KL_Loss = torch.mean(KL_tensor)
        ib_x1 = ib_x1.permute(0, 2, 1)  # 调整维度顺序
        return ib_x1, KL_Loss,p_3
...
class CNNModelWithVAE(nn.Module): 
    ...
    def forward(self, x):
        ...
        # 分别通过三个独立的CNN
        ib_x_3, KL_Loss,channal_importance_3= self.cnn1(x1,x2,x3)
        ...
        out = ib_x_3_t  # 残差连接，可根据需要再加 LayerNorm 等后处理
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, unit='batch', desc=f"Epoch {epoch+1}/{epochs}") as tepoch:
            for batch in tepoch:
                ...
                total_loss =pred_loss +   + 0.000000000001 * kl
                total_loss.backward()
